...
class ReviewHandler:
    ...
    def get_appreciation(self):
        ...
        try:
            nb_helpful = re.findall(
                "\t([0-9]*?) (people|person) found this review helpful",
                appreciation
                )[0][0]
        except IndexError:
            pass
        try:
            nb_funny = re.findall(
                "\t([0-9]*?) (people|person) found this review funny",
                appreciation)[0][0]
        except IndexError:
            pass
    ...
    def get_awards(self, info=True):
        ...
        for award in awards:
            ...
            award_name = re.findall(
                '<div class="reaction_award_name">(.*?)</div>', data
                )[0]
            award_count = re.findall(
                '<div class="reaction_award_count">(.*?)</div>', data
                )[0]
            award_points = re.findall(
                '<div class="reaction_award_points">(.*?)</div>', data
                )[0]
