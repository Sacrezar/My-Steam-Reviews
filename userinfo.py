...
class UserInfo:
    ...
    def is_api_key(self, api_key):
        call = "http://api.steampowered.com/ISteamUserStats/GetSchemaForGame"\
            f"/v2/?key={api_key}&appid=218620"
        return True if requests.get(call) else False

    def is_account_id(self, api_key, account_id):
        call = "http://api.steampowered.com/ISteamUser/GetPlayerSummaries"\
            f"/v0002/?key={api_key}&steamids={account_id}"
        is_account = requests.get(call).json()["response"]["players"]

        return True if len(is_account) >= 1 else False

    def request_account_id(self, api_key, vanity_url):
        call = "http://api.steampowered.com/ISteamUser/ResolveVanityURL"\
            f"/v0001/?key={api_key}&vanityurl={vanity_url}"
